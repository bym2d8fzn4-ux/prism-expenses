...
async function loadExpenses() {
  try {
    ...
    console.error(error);
    elements.preview.textContent = "Expenses could not load your saved entries for the report.";
...
function exportReport() {
  ...
  try {
    ...
    const filename = `expenses-report-${startDate}-to-${endDate}.xlsx`;
    ...
    if (tryShareReport(blob, filename)) {
      elements.status.textContent = "Report is ready. If the share sheet does not open, tap Open Report File.";
      return;
    }

    elements.downloadLink.click();
    elements.status.textContent = "Report is ready. If the download does not open, tap Open Report File.";
  } catch (error) {
    console.error(error);
    window.alert("Expenses could not export that report.");
  } finally {
    elements.exportButton.disabled = false;
    elements.exportButton.textContent = "Export Report";
  }
}
...
function tryShareReport(blob, filename) {
  ...
  navigator.share({
    ...
    if (error?.name !== "AbortError") {
      console.error("Expenses could not open the report share sheet.", error);
      elements.status.textContent = "Report is ready. Tap Open Report File to open it.";
    }
  });
...
function buildReportRows(startDate, endDate) {
  return getExpensesInReportRange(startDate, endDate).map((expense) => {
    ...
    return {
      ...
      paidDate: expense.reimbursedDate || "",
      airport: expense.location || "",
      aircraft: expense.aircraft || "",
      notes: expense.notes || "",
    };
  });
}
...
function getDetailedStatusLabel(expense, recordType = getRecordType(expense)) {
  if (expense.reimbursedDate) {
    return recordType === "incentive" ? "Paid incentive" : "Paid";
  }

  if (expense.submittedDate) {
    return recordType === "incentive" ? "Submitted incentive" : "Submitted";
  }
