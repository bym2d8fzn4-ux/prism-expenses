...
const elements = {
  ...
  submittedDateField: document.querySelector("#submitted-date-field"),
  submittedDate: document.querySelector("#submitted-date"),
  paidDateField: document.querySelector("#paid-date-field"),
...
async function loadExpense(id) {
  try {
    ...
    console.error(error);
    window.alert("Expenses could not load this entry.");
  }
}
...
function populateForm(expense) {
  ...
  setAmountInputValue(expense.amount);
  ...
  elements.aircraft.value = expense.aircraft || "";
...
async function handleSaveExpense(event) {
  ...
  const amount = parseAmountInputValue();
  ...
  const expense = {
    ...
    aircraft: elements.aircraft.value.trim().toUpperCase(),
  ...
  try {
    ...
    console.error(error);
    window.alert("Expenses could not save this entry.");
  }
}
...
function updateStatusDateVisibility(isEditing = Boolean(state.existingExpense)) {
  const hasSubmittedDate = Boolean(elements.submittedDate.value);
  const hasPaidDate = Boolean(elements.reimbursedDate.value);
  elements.submittedDateField.hidden = !isEditing && !hasSubmittedDate && !hasPaidDate;
  elements.paidDateField.hidden = !hasPaidDate;
}
...
async function handleAutofillReceipt(options = {}) {
  ...
  try {
    ...
    if (!options.autoTriggered) {
      window.alert("Expenses could not read that receipt clearly. You can still enter the fields manually.");
    }
  } finally {
...
function applyReceiptSuggestion(suggestion) {
  ...
  if (suggestion.amount && shouldApplySuggestion(elements.amount)) {
    setAmountInputValue(suggestion.amount);
...
function handleAmountInput() {
  const cents = Number(elements.amount.value.replace(/\D/g, "") || 0);
  elements.amount.dataset.cents = String(cents);
  elements.amount.value = formatCents(cents);
}

function setAmountInputValue(value) {
  const cents = Math.max(0, Math.round(Number(value || 0) * 100));
  elements.amount.dataset.cents = String(cents);
  elements.amount.value = cents ? formatCents(cents) : "";
}

function parseAmountInputValue() {
  const cents = Number(elements.amount.dataset.cents);
  if (Number.isFinite(cents) && cents > 0) {
    return cents / 100;
  }

  return Number.parseFloat(String(elements.amount.value || "").replace(/[^0-9.]/g, ""));
}
...
async function useCurrentLocation() {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    () => {
      window.alert("Expenses could not access your location.");
      elements.useLocationButton.disabled = false;
      elements.useLocationButton.textContent = "Use GPS";
    },
    {
